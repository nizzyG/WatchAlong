...
Looking for the main setup? Go back to the [README](README.md) or follow our visual [Step-by-Step Tutorial](tutorial.md).
...
See the [Step-by-Step Tutorial](tutorial.md#step-5-connect-to-patreon) for detailed visual guidance on connecting Patreon.
...
The initial sync is a quick manual process—you only do it once per pairing. After loading your movie and downloading the reaction, both videos open paused side‑by‑side with the **Sync Setup** panel visible. 

See the visual walkthrough in our [Step-by-Step Tutorial](tutorial.md#step-8-the-sync-setup-screen) for details. Here is a quick summary:
1. Press **Play** on the **reaction video** and watch the opening minutes. Reactors almost always do a "3… 2… 1… play" countdown when they start the film.
2. At the exact moment the countdown ends, click **Save Sync**. WatchAlong calculates the time offset between the two videos.
3. In the first minute or so, use the **`[` and `]` keys** to nudge the sync forward or backward by 0.1 seconds until it feels perfect.
4. That's it. From that point on, WatchAlong keeps everything locked—pausing, seeking, and restarting the app all maintain your sync point.
...
Open an issue on the [WatchAlong GitHub repository](https://github.com/nizzyG/WatchAlong/issues). Please describe what you were doing, what you expected to happen, and what happened instead. Screenshots are always helpful.
