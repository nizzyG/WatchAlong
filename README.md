...
| **Patreon download** | Connect your Patreon session right from your browser—no browser extensions, just a friendly guided flow. Your credentials never leave your device unless you explicitly opt in. |
...
| **Subtitle support** | Load SRT/VTT subtitle files for the movie, shown right inside the PiP overlay. |
...
- **A Patreon subscription** to a reactor who provides full‑length watchalong videos.

WatchAlong bundles `yt‑dlp` and `patreon‑dl` so you don’t need to install anything extra.

### Installation

1. Download the latest release from the [Releases](https://github.com/nizzyG/WatchAlong/releases) page.
2. On Windows, run the `.exe` installer. On macOS, open the `.dmg` and drag WatchAlong into Applications.
