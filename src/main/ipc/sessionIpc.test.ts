...
describe('session media IPC capabilities', () => {
  ...
  it('clears a manual poster only for a stored movie session', () => {
    ...
    expect(clearPoster({}, 'missing-session')).toEqual({ version: 8, activeSessionId: null, sessions: [], reactors: [] })
    expect(clearPoster({}, 'bad/session')).toEqual({ version: 8, activeSessionId: null, sessions: [], reactors: [] })
...
function createSessionStore() {
  return {
    read: vi.fn(() => ({ version: 8, activeSessionId: null, sessions: [], reactors: [] })),
